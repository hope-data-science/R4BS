...
if(F){
  ...
  r_query = "REF ( {R: A Language and Environment for Statistical Computing}  OR  {http://www.r-project.org} )  AND  DOCTYPE ( ar ) "
  ...
  # write_rds(subject_year,"data/subject_year_20210924.rds")
  ###
  search_list = do.call(paste,expand.grid(time_query,subject_query)) %>% 
...
# write_rds(subject_year_total,"data/subject_year_total_20210924.rds")

read_rds("data/subject_year_20210924.rds") -> subject_year
read_rds("data/subject_year_total_20210924.rds") -> subject_year_total
## only article
...
subject_year %>%
  ...
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90,hjust = 1,vjust = .5)) -> p1
...
# ggsave("figure/fig4_20211228.png",width = 10,height = 10)
